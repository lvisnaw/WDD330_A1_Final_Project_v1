//JSON URL
const apiURL = '//api.openweathermap.org/data/2.5/forecast?id=5779334&units=imperial&appid=f0af057df16329fc66d52dd104cebd2d'
...
fetch(apiURL)
    ...
    fetch(sodaEventURL)
        ...
        .then(function (sodaObject) {
            const towns = sodaObject['towns'];
            for (let i = 0; i < towns.length; i++) {
                if (towns[i].name == 'Soda Springs') {
                    ...
                    for (let x = 0; x < towns[i].events.length; x++) {
                        ...
                        soda.appendChild(info);
                        document.getElementById('soda').appendChild(soda);
                    }
                }
            }
